...
//获取cartItemId
      var id = $(this).attr("id").substring(0, 32);
        ...
        //<a class="jia" id="${cartItem.cartItemId }Jia">
...
function showTotal() {
	var total = 0;
	/*
	1. 获取所有的被勾选的条目复选框！循环遍历之全选中 checked=true
	*/
...
/*
 * 统一设置所有条目的复选按钮 bool 布尔值 true 就是对 对就是打个对号
 */
...
function jiesuan(){
    // 1. 获取所有被选中条目的复选框
	// 2. 创建一数组，把所有被选中的复选框的值添加到数组中 多个需用数组储存 到 dao 里有专门的 字符串分割

    
    var cartItemIdArray= new Array();
$(":checkbox[name=checkboxBtn][checked=true]").each(function() {
		cartItemIdArray.push($(this).val());//把复选框的值添加到数组中   cartItemId 值
		
	});
        //alert(cartItemIdArray); cid
...
function batchDelete(){
	...
	// 3. 指定location为CartItemServlet，参数method=batchDelete，参数cartItemIds=数组的toString()
            var cartItemIdArray = new Array();
       $(":checkbox[name=checkboxBtn][checked=true]").each(function() {  //checked=true 表示所有打对号的 然后遍历
...
<c:forEach items="${cartItemLIst }" var="cartItem">
	<tr align="center">
		...
		<td align="left" width="70px">
			<a href="<c:url value='/BookServlet?method=load&bid=${cartItem.book.bid}'/>"><img border="0" width="54" align="top" src="<c:url value='/${cartItem.book.image_b }'/>"/></a>
		</td>
